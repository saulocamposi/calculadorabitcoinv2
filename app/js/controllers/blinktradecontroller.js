...
app.controller('blinktradecontroller', ['$scope', '$http', 'blinktradeChartService', function($scope, $http, blinktradeChartService) {
  ...
  $http.get("https://api.blinktrade.com/api/v1/VEF/ticker?crypto_currency=BTC").then(function(response) {
  ...
  $http.get("https://api.blinktrade.com/api/v1/CLP/ticker?crypto_currency=BTC").then(function(response) {
    var datachilebit = response.data;
    dataChart.push({"c" : [{"v" : "Chilebit", "f": null},{"v" : datachilebit.vol, "f": null}]});
    $scope.chilebit = datachilebit;
  });

  $http.get("https://api.blinktrade.com/api/v1/PKR/ticker?crypto_currency=BTC").then(function(response) {

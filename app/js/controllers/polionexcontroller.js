...
var polionexcontroller = function($scope, $http, polionexChartService, googleChartApiPromise) {
    ...
    $http.get("http://localhost/calculadorabitcoinv2/app/services/doge_polionex_services.php").then(function(response) {
      var doge = response.data;
      dataChart.push({"c" : [{"v" : "Doge", "f": null},{"v" : doge.volume, "f": null}]});
      $scope.doge = doge;
    });

    $http.get("http://localhost/calculadorabitcoinv2/app/services/dash_polionex_services.php").then(function(response) {
      var dash = response.data;
      dataChart.push({"c" : [{"v" : "Dash", "f": null},{"v" : dash.volume, "f": null}]});
      $scope.dash = dash;
    });

    $http.get("http://localhost/calculadorabitcoinv2/app/services/lite_polionex_services.php").then(function(response) {
    ...
    $http.get("http://localhost/calculadorabitcoinv2/app/services/ether_polionex_services.php").then(function(response) {

...
app.controller('poloniexcontroller', ['$scope','$http', 'polionexChartService', function($scope, $http, polionexChartService) {

    var dataChart = [];

    var root_path = "backend/src/ws/services/poloniex/v1/";
    ...
    $http.get(root_path + "dash_poloniex_services.php").then(function(response) {
      var dash = response.data;
      $scope.dash = dash;
      dataChart.push({"c" : [{"v" : "Dash", "f": null},{"v" : dash.volume, "f": null}]});
    });

    $http.get(root_path + "lite_poloniex_services.php").then(function(response) {
      var lite = response.data;
      $scope.lite = lite;
      dataChart.push({"c" : [{"v" : "Lite", "f": null},{"v" : lite.volume, "f": null}]});
    });

    $http.get(root_path + "ether_poloniex_services.php").then(function(response) {
